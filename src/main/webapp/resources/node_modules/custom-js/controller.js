...
app.controller('createCustomerController', ['$scope', 'userInfo', function ($scope, userInfo) {
    ...
    initCreateCustomerUI($scope.validationError);
    initCreateCustomerUI($scope.customer);
    $scope.finderloader = false;
...
app.controller('updateCustomerController', ['$scope', 'userInfo', 'userMessageService', '$localStorage', '$window', function ($scope, userInfo, userMessageService, $localStorage, $window) {
    $scope.loaded = false;

    //load user list to UI
    $scope.List = userInfo.customerList(
        function (data) {
    ...
    //check box click load indivisual data to user UI
    $scope.loadCustomerInfo = function (obj, index) {
        loadCustomerInfo($scope, obj, index);
    }
    ...
    $scope.updateCustomer= function () {
        ...
        $scope.finderloader = true;
        //Create Customer Object from UI to send server side
        $scope.newObj = updateCustomerObj($scope.customer);
        console.log($scope.newObj);
        //Call service method to update User object
        userInfo.updateCustomer(function (data) {
                ...
                if (data.customer) {
                    ...
                    setCustomerUiValue($scope.customer, data.customer);
                    $scope.loaded = false;
                    //set server value to userList ui part
                    //setValueToUserList(data.user.id, data.user,$scope);
                    $scope.List = userInfo.customerList(
    ...
    $scope.deleteCustomer = function () {
        ...
        //Call service method to update Customer object
        userInfo.deleteCustomer(function (data) {
            //if logged user not found then redirect to login page
            if (data.invalidUserError) redirectToLogin($localStorage, data, $window, 'other');
            $scope.validationErrorMsg = data.validationError;
            $scope.successMsg = data.successMsg;
            if ($scope.successMsg) initUpdateUserUI($scope.user);
            deleteRow($scope.customer.id);
            $scope.finderloader = false;
        }, function (error) {
            $scope.internalError = getErrorMessage(error);
            $scope.finderloader = false;
...
function loadCustomerInfo(scope, obj, index) {
    scope.index = index;
    scope.customer.id = obj.id;
    scope.customer.version = obj.version;
    scope.customer.name = obj.name;
    scope.customer.customerCode = obj.customerCode;
    scope.customer.email = obj.email;
    scope.customer.phone = obj.phone;
    scope.customer.nid = obj.nid;
    scope.customer.address = obj.address;

}


function updateCustomerObj(fromUi) {
    var updateCustomerNewObj = {
        id: fromUi.id,
        version: fromUi.version,
        name: fromUi.name,
        customerCode: fromUi.customerCode,
        email: fromUi.email,
        phone: fromUi.phone,
        nid: fromUi.nid,
        address: fromUi.address
    ...
    return updateCustomerNewObj;
}

function setCustomerUiValue(ui, scope) {
    ui.id = scope.id;
    ui.version = scope.version;
    ui.name = scope.name;
    ui.customerCode = scope.customerCode;
    ui.email = scope.email;
    ui.phone = scope.phone;
    ui.nid = scope.nid;
    ui.address = scope.address;
